	...
	<view class="flow-box" :style="'height: ' + loadingTop + 'px'">
		<view class="item"
			...
			:data-index="index"
			 @click="choose">
			<view class="pic">
				<image class="image" mode="widthFix" :src="item.bookimgs" style="width: 100%; height: 100%; display: block;" ></image>
			</view>
			<view class="content">
				<text>{{item.bookmsgs}}</text>
				<view class="user">
					<image style="width: 40upx; height: 40upx; border-radius: 50%; margin-right: 10upx;" 
						:src="item.bookimgs"></image>
					{{item.bookname}}
				</view>
			</view>
		</view>	
		<view class="loading" v-show="loading" :style="'top: ' + loadingTop + 'px'" >
			<image src="/static/img/timg.gif" style="width: 80upx; height: 80upx;"></image>
		</view>
	</view>
</template>
